    ...
    try:
        with open(img_path, "rb") as img, open(mask_path, "rb") as mask:
            rsp = await _client.images.edit(
                ...
                prompt=(
                    # 1. Стиль
                    "Studio Ghibli portrait, gentle anime line-art, soft watercolor shading, "
                    "warm pastel palette, subtle film-grain texture. "
                    # 2. Ракурс и композиция
                    "Front-facing bust, same framing and lighting as source photo, plain light background. "
                    # 3. Что оставить без изменений
                    "Keep the person’s exact face shape, glasses, haircut, eye colour and neutral expression; "
                    "no change to pose or proportions."
                ),
                n=1,
                size="1024x1024",
